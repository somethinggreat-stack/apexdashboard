...
- **All four steps are built** (see below). **Retention caps all of it at 7 days.**

### Owner controls — Step 2 of 4: the owner can act (2026-09-19)
The owner runs every group in their own org and can remove any message. The line that holds the whole design: **acting is allowed and visible; reading is not.**

- `ownsOrg(Admin, Conversation)` = super admin **and** the conversation belongs to their org. Used ONLY in action guards — never for reading. `test_the_owner_still_cannot_read_a_thread_they_are_not_in` pins that: `thread` and `open` still 403 for a group the owner isn't in, even though they can delete a message in it.
- `authorizeGroupAdmin` short-circuits for the owner, so rename / add members / remove member work on **any** group in the org, including ones they were never added to. `open()`'s `isAdmin` flag is true for the owner too, so the members panel appears.
- `destroy()` — the owner may delete for everyone anywhere in their org. "Delete for me" still requires being in the conversation (it makes no sense otherwise). `deleted_by` is stamped, so the bubble reads **"This message was deleted by Umair"** — never a silent vanish. Client: `IS_OWNER` only decides whether the option is offered; the server decides whether it happens.
- New owner-only endpoints: `group/{conversation}/join` (adds the owner as admin and posts a **visible** "joined the group" system message — stepping into a conversation is never silent) and `people/{admin}/remove-everywhere` (offboarding: out of every group at once; DMs are left alone because a DM is a pair and clearing one would take the other person's history with it). Both `->middleware('admin.super')`.
- Logging: join and remove-everywhere go through the `LogActivity` allowlist. Message deletion is **not** in that allowlist — that would record every VA hiding a message for themselves — so `destroy()` writes its own entry, only when the owner removes **someone else's** message, naming the sender and the conversation.
- Two old tests in `TeamMessageTest` asserted the owner could NOT delete others' messages; they encoded the pre-step-2 rule and were rewritten to assert a **VA** cannot. That was an intended behaviour change, not a regression.
- Covered by `TeamChatOwnerControlsTest` (12 tests) and `phase11` (13 checks, including the sender seeing "deleted by Umair").

### Owner controls — Steps 3 and 4 of 4: announcements and records (2026-09-19)
- **Step 3 — one message to the whole team.** `POST team-messages/announce` (super only) drops a copy into **each person's existing direct chat with the owner** rather than into a new "announcements" channel. That was the key decision: it is where they already look, and unread badges, desktop notifications and read watermarks all work with no new machinery. Every copy carries the same `announcement_id` (migration `2026_09_26_000001`, nullable + indexed, and added to `TeamMessage::$fillable` — that was the one thing that silently dropped it at first). The overview then reports **"read by N of M"** and names who hasn't: a person has read it when their side of that DM has been read past their copy's id.
- **Step 4 — records.**
  - `team-messages/overview/files` — every file that has gone out through the chat across the org: name, size, sender, which conversation, when. It answers "which client files left this company, and to whom". Listing only: **opening** a file still goes through the ordinary attachment route, which requires being in the conversation.
